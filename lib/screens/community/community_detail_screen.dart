...
class _CommunityDetailScreenState extends State<CommunityDetailScreen> {
  ...
  Future<void> _pickAndUploadImage({required bool isBanner}) async {
    ...
    final croppedFile = await ImageCropper().cropImage(
      ...
      uiSettings: [
        AndroidUiSettings(toolbarTitle: isBanner ? 'Crop Banner' : 'Crop Icon', toolbarColor: TwitterTheme.blue, toolbarWidgetColor: Colors.white, lockAspectRatio: true),
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final List editors = (data['editors'] is List) ? data['editors'] : [];
        final List followers = (data['followers'] is List) ? data['followers'] : [];
                  ...
                  flexibleSpace: FlexibleSpaceBar(
                    background: Stack(
                      ...
                      children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        Flexible(child: Text(name, style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
                                        if (isVerified) ...[SizedBox(width: 4), Icon(Icons.verified, size: 18, color: TwitterTheme.blue)],
                                      ],
                                    ),
                                    Text(
                                      category == 'pnj_official' ? "Official Campus Channel" : (category == 'partner_official' ? "Official Partner" : "Community Channel"),
                        ...
                        if (_isUploadingImage) Center(child: CircularProgressIndicator())
                      ],
                    ),
                  ),
                ),
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            if (!canPost)
                              ElevatedButton(
                                onPressed: () => _handleFollowAction(isFollower),
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: isFollower ? theme.cardColor : TwitterTheme.blue,
                                  foregroundColor: isFollower ? theme.textTheme.bodyLarge?.color : Colors.white,
                                  elevation: 0,
                                  side: isFollower ? BorderSide(color: theme.dividerColor) : null,
                                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))
                                ),
                                child: Text(isFollower ? "Following" : "Follow"),
                              )
                            else
                              ElevatedButton.icon(
                                onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(
                                    builder: (_) => CreatePostScreen(initialData: {
                                      'communityId': widget.communityId,
                                      'communityName': name,
